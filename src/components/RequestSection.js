...
import { FaFemale, FaMale, FaRegHospital } from "react-icons/fa";
import { updateRequest } from "@/actions/request";



export default function Doctorrequests({ requests, session }) {
  ...
  const handleAction = (type, requestsId) => {
    setSelectedAction({ type, requestsId });
    setDialogOpen(true);
  };
  const confirmAction = async () => {
    if (selectedAction.type === "accept") {
      console.log("selectedAction=>", selectedAction);
      await updateRequest(selectedAction.requestsId, "accepted");
    } else if (selectedAction.type === "reject") {
      console.log("selectedAction=>", selectedAction);
      await updateRequest(selectedAction.requestsId, "rejected");
    }
    setDialogOpen(false);
  };
  ...
  const renderrequestsCard = (request) => (
    <Card key={request._id}>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
              {request.gender === "male" ? <FaMale /> : <FaFemale />}
      ...
      <CardFooter className="justify-between">
        <DoctorDetailSheet session={session} requests={requests} />
        {request.status === "rejected" ? (
          ...
          >
            ...
            <span className="sr-only">Rejected doctor requests</span>
          </Button>
          ...
          >
            ...
            <span className="sr-only">Accepted doctor requests</span>
          </Button>
        ) : (
      ...
      <Tabs defaultValue="all" className="w-full mb-6">
        ...
        <TabsContent value="all">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredrequests.map(renderrequestsCard)}
          </div>
        </TabsContent>
